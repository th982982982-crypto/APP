...
export default function Dashboard() {
  ...
  const [showTotalBalanceModal, setShowTotalBalanceModal] = useState(false);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (transRes.error) {
        if (transRes.error.message.includes('relation "public.transactions" does not exist')) {
          setDbError('Bảng "transactions" chưa được tạo. Vui lòng chạy SQL setup.');
        }
        throw transRes.error;
      }
      if (catsRes.error) {
        if (catsRes.error.message.includes('relation "public.categories" does not exist')) {
          setDbError('Bảng "categories" chưa được tạo. Vui lòng chạy SQL setup.');
        }
        throw catsRes.error;
      }
      if (walletsRes.error) {
        if (walletsRes.error.message.includes('relation "public.wallets" does not exist')) {
          setDbError('Bảng "wallets" chưa được tạo. Vui lòng chạy SQL setup.');
        }
  ...
  const confirmDeleteWallet = async () => {
    ...
    try {
      const { error } = await supabase.from('wallets').delete().eq('id', deletingWalletId);
      if (error) throw error;
      setWallets(prev => prev.filter(w => w.id !== deletingWalletId));
      if (selectedWallet?.id === deletingWalletId) setSelectedWallet(null);
  ...
  if (dbError) {
    return (
      <div className="flex flex-col items-center justify-center p-20 liquid-glass text-center space-y-6">
        ...
        <h3 className="text-2xl font-black text-white">Lỗi Cơ Sở Dữ Liệu</h3>
        <p className="text-white/60 max-w-md">{dbError}</p>
        <div className="bg-black/40 p-6 rounded-3xl border border-white/10 text-left w-full max-w-2xl overflow-x-auto">
          <p className="text-emerald-400 font-mono text-xs mb-4">-- Vui lòng chạy SQL này trong Supabase SQL Editor:</p>
          <pre className="text-white/40 font-mono text-[10px] leading-relaxed">
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <motion.div 
          ...
          onClick={() => setShowTotalBalanceModal(true)}
          className="liquid-glass p-10 relative overflow-hidden group hover:shadow-[0_20px_80px_rgba(16,185,129,0.15)] transition-all duration-500 cursor-pointer"
        >
        ...
        <motion.div 
          ...
          className="liquid-glass p-10 relative overflow-hidden group hover:shadow-[0_20px_80px_rgba(244,63,94,0.15)] transition-all duration-500 cursor-pointer"
        >
          <div className="absolute top-4 right-4 p-2 opacity-0 group-hover:opacity-100 transition-opacity">
            <LayoutGrid className="w-5 h-5 text-white/40" />
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              <button 
                ...
                className="text-xs font-black text-white/30 uppercase tracking-widest hover:text-white transition-colors flex items-center gap-2"
              >
        ...
        {showTotalBalanceModal && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
            <motion.div 
              ...
              exit={{ opacity: 0 }}
              className="absolute inset-0 bg-black/60 backdrop-blur-xl" 
              onClick={() => setShowTotalBalanceModal(false)}
            ></motion.div>
            <motion.div 
              initial={{ opacity: 0, scale: 0.9, y: 20 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.9, y: 20 }}
              className="relative w-full max-w-2xl bg-white/[0.03] backdrop-blur-3xl border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.3)] rounded-[3rem] p-6 md:p-10 overflow-y-auto max-h-[85vh] transition-all duration-500"
            >
              <div className="flex items-center justify-between mb-10">
                <div>
                  <h3 className="text-3xl font-black text-white tracking-tight">Chi tiết tài sản</h3>
                  <p className="text-white/40 text-xs font-bold uppercase tracking-widest mt-1">Phân bổ số dư trên các ví</p>
                </div>
                <button 
                  onClick={() => setShowTotalBalanceModal(false)}
                  className="p-3 text-white/20 hover:text-white hover:bg-white/5 rounded-2xl transition-all"
              ...
              <div className="space-y-6">
                <div className="liquid-glass p-8 text-center">
                  <p className="text-white/40 text-[10px] font-black uppercase tracking-widest mb-2">Tổng tài sản hiện có</p>
                ...
                <div className="grid grid-cols-1 gap-4">
                  {wallets.map(wallet => (
                    <div key={wallet.id} className="liquid-glass p-6 flex items-center justify-between group hover:bg-white/5 transition-all">
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-2xl flex items-center justify-center bg-white/5" style={{ color: wallet.color }}>
                          <Wallet className="w-6 h-6" />
                        </div>
                        <div>
                          <p className="text-lg font-black text-white">{wallet.name}</p>
                          <p className="text-[10px] font-black text-white/30 uppercase tracking-widest">
                            {((wallet.balance / (wallets.reduce((acc, w) => acc + w.balance, 0) || 1)) * 100).toFixed(1)}% tổng tài sản
                          </p>
                        </div>
                      </div>
                      <p className="text-xl font-black text-white tracking-tighter">{formatCurrency(wallet.balance)}</p>
                    </div>
                  ))}
                </div>
              </div>
        ...
        {showBudgetModal && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
            ...
            >
              <div className="flex items-center justify-between mb-10">
                <div>
                  <h3 className="text-3xl font-black text-white tracking-tight">Hạn mức chi tiêu</h3>
                  <p className="text-white/40 text-xs font-bold uppercase tracking-widest mt-1">Theo dõi ngân sách hàng tháng</p>
                </div>
                <button 
                  onClick={() => setShowBudgetModal(false)}
                  className="p-3 text-white/20 hover:text-white hover:bg-white/5 rounded-2xl transition-all"
              ...
              <div className="space-y-8">
                {wallets.filter(w => w.monthly_budget).length > 0 ? (
                  wallets.filter(w => w.monthly_budget).map(wallet => {
                    const spent = Math.abs(transactions.filter(t => t.wallet_id === wallet.id && t.type === 'expense').reduce((acc, t) => acc + t.amount, 0));
                    const percentage = Math.min((spent / (wallet.monthly_budget || 1)) * 100, 100);
                    const isOver = spent > (wallet.monthly_budget || 0);

                    return (
                      <div key={wallet.id} className="liquid-glass p-8 space-y-6">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-4">
                            <div className="w-12 h-12 rounded-2xl flex items-center justify-center bg-white/5" style={{ color: wallet.color }}>
                              <Wallet className="w-6 h-6" />
                            ...
                            <div>
                              <p className="text-xl font-black text-white">{wallet.name}</p>
                              <p className="text-[10px] font-black text-white/30 uppercase tracking-widest">Hạn mức: {formatCurrency(wallet.monthly_budget || 0)}</p>
                            </div>
                          </div>
                          <div className="text-right">
                            <p className={`text-xl font-black tracking-tighter ${isOver ? 'text-rose-400' : 'text-white'}`}>
                              {formatCurrency(spent)}
                            </p>
                            <p className="text-[10px] font-black text-white/30 uppercase tracking-widest">Đã chi tiêu</p>
                          </div>
                        </div>

                        <div className="space-y-3">
                          <div className="flex justify-between text-[10px] font-black uppercase tracking-widest">
                            <span className="text-white/30">Tiến độ</span>
                            <span className={isOver ? 'text-rose-400' : 'text-emerald-400'}>{percentage.toFixed(1)}%</span>
                          </div>
                          <div className="w-full bg-white/5 h-3 rounded-full overflow-hidden p-0.5 border border-white/5">
                            <motion.div 
                              initial={{ width: 0 }}
                              animate={{ width: `${percentage}%` }}
                              className={`h-full rounded-full shadow-[0_0_15px_rgba(0,0,0,0.5)] transition-all duration-1000`}
                              style={{ backgroundColor: isOver ? '#f43f5e' : wallet.color }}
                            ></motion.div>
                          </div>
                          <div className="flex justify-between text-[9px] font-bold text-white/20 italic">
                            <span>0đ</span>
                            <span>{formatCurrency(wallet.monthly_budget || 0)}</span>
                          </div>
                        </div>
                      </div>
                    );
                  ...
                  <div className="text-center p-20 text-white/20 italic">
                    Chưa có ví nào được thiết lập hạn mức chi tiêu
                  </div>
                )}
              </div>
            </motion.div>
        ...
        {showWalletsModal && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.9, y: 20 }}
              className="relative w-full max-w-4xl bg-white/[0.03] backdrop-blur-3xl border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.3)] rounded-[3rem] p-6 md:p-10 overflow-y-auto max-h-[85vh] transition-all duration-500"
            >
              <div className="flex items-center justify-between mb-10">
                <div>
                  <h3 className="text-3xl font-black text-white tracking-tight">Quản lý ví</h3>
                  <p className="text-white/40 text-xs font-bold uppercase tracking-widest mt-1">Danh sách tất cả các tài khoản</p>
                </div>
                <div className="flex items-center gap-4">
                  <button 
                    onClick={() => {
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {wallets.map(wallet => (
                  <div key={wallet.id} className="liquid-glass p-8 space-y-6 group relative overflow-hidden">
                    <div className="flex items-center justify-between relative z-10">
                      <div className="flex items-center gap-4">
                        <div className="w-14 h-14 rounded-2xl flex items-center justify-center bg-white/5 shadow-inner" style={{ color: wallet.color }}>
                          <Wallet className="w-7 h-7" />
                        </div>
                        <div>
                          <h4 className="text-xl font-black text-white">{wallet.name}</h4>
                          <p className="text-[10px] font-black text-white/30 uppercase tracking-widest">Ngày tạo: {new Date(wallet.created_at).toLocaleDateString('vi-VN')}</p>
                        </div>
                      </div>
                      <button 
                        onClick={() => deleteWallet(wallet.id)}
                        className="p-3 text-rose-400/30 hover:text-rose-400 hover:bg-rose-500/10 rounded-2xl transition-all"
                      >
                        <Trash2 className="w-5 h-5" />
                      </button>
                    </div>
                    ...
                    <div className="space-y-1 relative z-10">
                      <p className="text-[10px] font-black text-white/30 uppercase tracking-widest">Số dư hiện tại</p>
                      <p className="text-3xl font-black text-white tracking-tighter">{formatCurrency(wallet.balance)}</p>
                    </div>

                    {wallet.monthly_budget && (
                      <div className="pt-4 border-t border-white/5 relative z-10">
                        <div className="flex justify-between text-[10px] font-black uppercase tracking-widest mb-3">
                          <span className="text-white/30">Hạn mức tháng</span>
                          <span className="text-white">{formatCurrency(wallet.monthly_budget)}</span>
                        </div>
                    ...
                    {/* Background Glow */}
                    <div className="absolute -bottom-10 -right-10 w-32 h-32 blur-[80px] opacity-10 rounded-full" style={{ backgroundColor: wallet.color }}></div>
                  </div>
                ))}
              </div>
            </motion.div>
          ...
          <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
            ...
            >
              ...
              <p className="text-white/40 text-sm font-bold mb-8">
                {deletingWalletId ? 'Xóa ví sẽ ảnh hưởng đến các giao dịch liên quan.' : 'Hành động này không thể hoàn tác.'}
              </p>
              <div className="flex gap-4">
                ...
                <button
                  onClick={deletingWalletId ? confirmDeleteWallet : confirmDelete}
